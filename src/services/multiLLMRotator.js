const ROTATABLE_STATUS_CODES = new Set([403, 429, 500, 502, 503, 504])
...
export class MultiLLMRotator {
  ...
  async executeRequest(payload, fetchImpl = globalThis.fetch) {
    ...
    for (const key of available) {
      let response
      try {
        response = await requestProvider(key, payload, fetchImpl)
      } catch (error) {
...
async function requestProvider(config, payload, fetchImpl) {
...
export function createEnvironmentKeyPool(environment = process.env) {
  ...
  ].filter((entry) => entry.key && (entry.provider !== 'deepseek' || environment.ENABLE_PAID_DEEPSEEK === 'true'))
}
